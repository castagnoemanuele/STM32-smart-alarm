...
void TestSPI_Only(void);
void RFID_HardwareInit(void);
void Test_SPI3_I2C1(void);
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	// Enable ESP32
		...
		HAL_Delay(15000); // Give time to the ESP to boot
	...
	//Initialize RFID
	void RFID_HardwareInit();
	void TestSPI_Only();
		HAL_NVIC_SetPriority(SPI3_IRQn, 1, 0); //Raise SPI priority
		HAL_NVIC_SetPriority(I2C1_EV_IRQn, 2, 0);
		HAL_NVIC_SetPriority(I2C1_ER_IRQn, 2, 0);
		MFRC522_Init();


		// Test di comunicazione con l'RFID
		printf("\nSearching for an RC522\r\n");
		RFIDstatus = Read_MFRC522(VersionReg);
		printf("MFRC522 Version: 0x%02X\r\n", RFIDstatus);

		// Leggi e stampa più registri per debug
		printf("ControlReg: 0x%02X\r\n", Read_MFRC522(ControlReg));
		printf("CommIEnReg: 0x%02X\r\n", Read_MFRC522(CommIEnReg));
		printf("DivIEnReg: 0x%02X\r\n", Read_MFRC522(DivlEnReg));
		printf("TxControlReg: 0x%02X\r\n", Read_MFRC522(TxControlReg));

		// Test semplice di scrittura/lettura
		Write_MFRC522(FIFODataReg, 0xAA);
		uint8_t testRead = Read_MFRC522(FIFODataReg);
		printf("Test write/read: wrote 0xAA, read 0x%02X\r\n", testRead);
...
void TestSPI_Only(void) {
    printf("\n\n*** TEST SPI BASIC COMMUNICATION ***\n");

    // 1. Reimposta SPI con configurazione minima
    ...
    // 3. Test base SPI senza comunicare con l'RC522
    printf("Testing raw SPI with loopback...\n");
    uint8_t tx_test = 0xA5;
    uint8_t rx_test = 0;

    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_SET); // Disattiva CS
    HAL_SPI_TransmitReceive(&hspi3, &tx_test, &rx_test, 1, 100);
    printf("  Loopback test: sent 0x%02X, received 0x%02X\n", tx_test, rx_test);

    // 4. Test comunicazione base con l'RC522
    printf("Testing basic read of VERSION register...\n");

    // Abbassa CS
    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_RESET);
    HAL_Delay(5);

    // Invia comando di lettura del registro Version
    uint8_t addr_cmd = ((0x37 << 1) & 0x7E) | 0x80; // VersionReg + bit di lettura
    uint8_t dummy = 0x00;
    uint8_t rx_addr = 0;
    uint8_t rx_data = 0;

    printf("  Sending command 0x%02X\n", addr_cmd);
    if (HAL_SPI_TransmitReceive(&hspi3, &addr_cmd, &rx_addr, 1, 100) != HAL_OK) {
        printf("  SPI transmit error\n");
    }

    printf("  Sending dummy byte for data\n");
    if (HAL_SPI_TransmitReceive(&hspi3, &dummy, &rx_data, 1, 100) != HAL_OK) {
    ...
    printf("  Raw version register value: 0x%02X\n", rx_data);
...
// Reset e inizializzazione SPI dedicata per RFID
void RFID_HardwareInit(void) {
    // Configura SPI3 con impostazioni molto conservative
    hspi3.Init.Mode = SPI_MODE_MASTER;
    hspi3.Init.Direction = SPI_DIRECTION_2LINES;
    hspi3.Init.DataSize = SPI_DATASIZE_8BIT;
    hspi3.Init.CLKPolarity = SPI_POLARITY_LOW;
    hspi3.Init.CLKPhase = SPI_PHASE_1EDGE;
    hspi3.Init.NSS = SPI_NSS_SOFT;
    hspi3.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_256; // Più lento possibile
    hspi3.Init.FirstBit = SPI_FIRSTBIT_MSB;
    hspi3.Init.TIMode = SPI_TIMODE_DISABLE;
    hspi3.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;
    if (HAL_SPI_Init(&hspi3) != HAL_OK) {
    ...
    // Esegui un reset hardware completo
    printf("Eseguo reset hardware completo RFID...\n");
    HAL_GPIO_WritePin(RC522_Rst_GPIO_Port, RC522_Rst_Pin, GPIO_PIN_RESET);
    HAL_Delay(200); // Reset più lungo
    HAL_GPIO_WritePin(RC522_Rst_GPIO_Port, RC522_Rst_Pin, GPIO_PIN_SET);
    HAL_Delay(200); // Attesa più lunga

    // Verifica che i pin SPI funzionino
    printf("Test pin SPI:\n");

    // Abbassa e alza il CS per verificare che funzioni
    printf("Test CS...\n");
    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_RESET);
    HAL_Delay(10);
    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_SET);
    ...
    // Prova ad inviare un byte per verificare che MOSI e SCK funzionino
    printf("Test invio dati...\n");
    uint8_t test_data = 0xAA;
    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_RESET);
    HAL_SPI_Transmit(&hspi3, &test_data, 1, 100);
    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_SET);

    printf("Hardware RFID inizializzato\n");
}

void Test_SPI3_I2C1(void) {
    printf("Testing SPI3 and I2C1 together...\n");

    // Test I2C1
    uint8_t i2c_data[] = {0x01, 0x02, 0x03};
    HAL_StatusTypeDef i2c_status = HAL_I2C_Master_Transmit(&hi2c1, 0x50 << 1, i2c_data, sizeof(i2c_data), 100);
    ...
    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_RESET);
    HAL_StatusTypeDef spi_status = HAL_SPI_TransmitReceive(&hspi3, &spi_tx_data, &spi_rx_data, 1, 100);
    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_SET);
    if (spi_status == HAL_OK) {
        printf("SPI3 test passed, received: 0x%02X\n", spi_rx_data);
    } else {
        printf("SPI3 test failed\n");
    }
}
